...
public class FirstPersonController : NetworkBehaviour
{
    ...
    private void UpdateAnimations()
    {
        ...
        // Update local animation parameters
        ...
        // Only update if there's a change to prevent unnecessary network traffic
    ...
    private void RpcUpdateAnimationStates(bool walking, bool sprinting, bool crouched)
    {
        ...
        //animations.SetBool("crouch", crouched);
    }
    private void Awake()
    {
    ...
    private void Update()
    {
        ...
        if (isLocalPlayer & !us.canWrite)
        {
            if (Input.GetKeyDown(destroyKey))
            {
                if (escaped)
                {
                    gameObject.GetComponent<userSettingNotCam>().one();
                    escaped = false;
                    isWalking = false;
                    Cursor.lockState = CursorLockMode.Locked;
                ...
                {
                    gameObject.GetComponent<userSettingNotCam>().two();
                    escaped = true;
                    isWalking = false;
...
// Custom Editor
    ...
    private SerializedProperty canvasi;
    SerializedObject SerFPC;
    SerializedObject water;

    private void OnEnable()
    {
        fpc = (FirstPersonController)target;
        SerFPC = new SerializedObject(fpc);
        canvasi = serializedObject.FindProperty("canvasi");
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("By Jess Case. Modified by EpicSUS", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        GUILayout.Label("version 1.0.1", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();

        serializedObject.Update();

        EditorGUILayout.PropertyField(canvasi, true);
        ...
        EditorGUILayout.Space();
        fpc.us = (userSettings)EditorGUILayout.ObjectField(new GUIContent("US", "Usersettings"), fpc.us, typeof(userSettings), true);
        fpc.crosshairObject = (Image)EditorGUILayout.ObjectField(new GUIContent("crosshair", "Object"), fpc.crosshairObject, typeof(Image), true);
        fpc.playerCamera = (Camera)EditorGUILayout.ObjectField(new GUIContent("Camera", "Camera attached to the controller."), fpc.playerCamera, typeof(Camera), true);
        fpc.fov = EditorGUILayout.Slider(new GUIContent("Field of View", "The camera’s view angle. Changes the player camera directly."), fpc.fov, fpc.zoomFOV, 179f);
        fpc.cameraCanMove = EditorGUILayout.ToggleLeft(new GUIContent("Enable Camera Rotation", "Determines if the camera is allowed to move."), fpc.cameraCanMove);
        fpc.waterVolume = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Water volume", "water effect really"), fpc.waterVolume, typeof(GameObject), true);
        ...
        EditorGUILayout.Space();
        GUILayout.Label("Footsteps", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));

        fpc.enableFootsteps = EditorGUILayout.ToggleLeft(new GUIContent("Enable Footsteps", "Determines if footstep sounds will play when walking."), fpc.enableFootsteps);

        GUI.enabled = fpc.enableFootsteps;
        fpc.footstepAudioSource = (AudioSource)EditorGUILayout.ObjectField(new GUIContent("Audio Source", "AudioSource component to play footstep sounds."), fpc.footstepAudioSource, typeof(AudioSource), true);
        // In the Footsteps section of the editor, add this after the crouch volume multiplier field
        fpc.animations = (Animator)EditorGUILayout.ObjectField(new GUIContent("Animator", "Animator component for player animations"), fpc.animations, typeof(Animator), true);
        EditorGUILayout.LabelField("Footstep Sounds", EditorStyles.boldLabel);
        EditorGUI.indentLevel++;
        SerializedProperty footsteps = SerFPC.FindProperty("footstepSounds");
        EditorGUILayout.PropertyField(footsteps, new GUIContent("Clips", "Array of footstep sound clips."), true);
        EditorGUI.indentLevel--;

        fpc.footstepIntervalWalk = EditorGUILayout.Slider(new GUIContent("Walk Interval", "Time between footsteps when walking."), fpc.footstepIntervalWalk, 0.1f, 1f);
        fpc.footstepIntervalRun = EditorGUILayout.Slider(new GUIContent("Run Interval", "Time between footsteps when running."), fpc.footstepIntervalRun, 0.1f, 0.5f);
        fpc.footstepVolume = EditorGUILayout.Slider(new GUIContent("Volume", "Volume of footstep sounds."), fpc.footstepVolume, 0f, 1f);
        fpc.crouchFootstepVolumeMultiplier = EditorGUILayout.Slider(new GUIContent("Crouch Volume Multiplier", "Multiplier for footstep volume when crouched."), fpc.crouchFootstepVolumeMultiplier, 0f, 1f);
        GUI.enabled = true;
