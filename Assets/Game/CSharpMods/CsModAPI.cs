...
public static class CsModAPI
{
    ...
    public static GameObject LoadGameObjectBundlesFull(string fullPath)
    {
        if (File.Exists(fullPath) == false)
            throw new FileNotFoundException("Assetbundle file not founded!", Path.GetFileNameWithoutExtension(fullPath));
        ...
        GameObject loadedObject = assetBan.LoadAsset<GameObject>(Path.GetFileNameWithoutExtension(fullPath));
    ...
    public static GameObject LoadGameObjectBundles(string path)
    {
        string fullPath = Path.Combine(CsModsManager.mods[Assembly.GetCallingAssembly()].modDir, path);

        return LoadGameObjectBundlesFull(fullPath);
    }
